...
var path__default = /*#__PURE__*/_interopDefaultLegacy(path$e);
var path__namespace = /*#__PURE__*/_interopNamespace(path$e);
var url__default = /*#__PURE__*/_interopDefaultLegacy(url);
var process__default = /*#__PURE__*/_interopDefaultLegacy(process$1);
var fs__default = /*#__PURE__*/_interopDefaultLegacy(fs$a);
var fs__namespace = /*#__PURE__*/_interopNamespace(fs$a);
...
var jsImports__default = /*#__PURE__*/_interopDefaultLegacy(jsImports);
...
const toPath$2 = urlOrPath => urlOrPath instanceof URL ? url.fileURLToPath(urlOrPath) : urlOrPath;
...
async function locatePath(
	...
	const statFunction = allowSymlinks ? fs$a.promises.stat : fs$a.promises.lstat;
...
async function findUpMultiple(name, options = {}) {
	let directory = path__default["default"].resolve(toPath$1(options.cwd) || '');
...
function pauseStreams (streams, options) {
  if (!Array.isArray(streams)) {
    ...
    if (!streams._readableState && streams.pipe) {
      streams = streams.pipe(PassThrough$1(options));
    }
...
Object.defineProperty(path$d, "__esModule", { value: true });
path$d.removeLeadingDotSegment = path$d.escape = path$d.makeAbsolute = path$d.unixify = void 0;
...
path$d.unixify = unixify;
function makeAbsolute(cwd, filepath) {
    return path$c.resolve(cwd, filepath);
}
path$d.makeAbsolute = makeAbsolute;
function escape$1(pattern) {
    return pattern.replace(UNESCAPED_GLOB_SYMBOLS_RE, '\\$2');
}
path$d.escape = escape$1;
...
path$d.removeLeadingDotSegment = removeLeadingDotSegment;
...
function isAffectDepthOfReadingPattern(pattern) {
    const basename = path$9.basename(pattern);
...
async function isType(fsStatType, statsMethodName, filePath) {
	...
	try {
		const stats = await promisify$1(fs$1[fsStatType])(filePath);
...
const getPath = (filepath, cwd) => {
	...
	return path$3.isAbsolute(pth) ? pth : path$3.join(cwd, pth);
};

const addExtensions = (file, extensions) => {
	if (path$3.extname(file)) {
		return `**/${file}`;
	}
...
const getGlob = (directory, options) => {
	...
	if (options.files && options.extensions) {
		return options.files.map(x => path$3.posix.join(directory, addExtensions(x, options.extensions)));
	}

	if (options.files) {
		return options.files.map(x => path$3.posix.join(directory, `**/${x}`));
	}

	if (options.extensions) {
		return [path$3.posix.join(directory, `**/*.${getExtensions(options.extensions)}`)];
	}

	return [path$3.posix.join(directory, '**')];
};
...
function urlDirname (url) {
  return require$$0$1.dirname(url$1.fileURLToPath(url))
}

function urlJoin (url, ...str) {
  return require$$0$1.join(urlDirname(url), ...str)
}
...
const signal=signals.find(({name})=>require$$2.constants.signals[name]===number);
...
const setExitHandler = async (spawned, {cleanup, detached}, timedPromise) => {
	...
	const removeExitHandler = onExit__default["default"](() => {
		spawned.kill();
	});
...
async function executeVueTsc(args, execaOptions) {
    const vueTscWrapperPath = urlJoin((typeof document === 'undefined' ? new (require('u' + 'rl').URL)('file:' + __filename).href : (document.currentScript && document.currentScript.src || new URL('index.cjs', document.baseURI).href)), '../vue-tsc/wrapper.js');
    const { stdout: vueSFCDeclaration } = await execa('node', [vueTscWrapperPath, ...args], execaOptions);
    return vueSFCDeclaration;
}

async function compileVueSFC(options) {
    options = {
        vueExtension: true,
        ...options,
    };
    ...
    if (options.projectRootPath === undefined) {
        ...
        if (pkgJsonPath === undefined) {
            throw new Error('Could not find the root of the project (please set the option `projectRootPath` to the path of your project root).');
        }
        projectPath = path__namespace.dirname(pkgJsonPath);
    }
    else {
    ...
    if (options.declarations) {
        ...
        if (options.tsconfigPath === undefined) {
            const projectTsconfigPath = await findUp('tsconfig.json', {
                cwd: path__namespace.dirname(vueSFCFiles[0]),
            });
        ...
        await Promise.all(vueSFCFiles.map(async (vueSFCFilePath) => {
            ...
            const relativePathDir = path__namespace.dirname(path__namespace.relative(projectPath, vueSFCFilePath));
            const vueSFCName = path__namespace.parse(vueSFCFilePath).name;
            const vueSFCDtsFileName = options.vueExtension
                ? `${vueSFCName}.vue.d.ts`
                : `${vueSFCName}.d.ts`;
            if (options.outDir !== undefined) {
                const vueSFCDtsPath = path__namespace.join(options.outDir, relativePathDir, vueSFCDtsFileName);
                if (!fs__namespace.existsSync(path__namespace.dirname(vueSFCDtsPath))) {
                    fs__namespace.mkdirSync(path__namespace.dirname(vueSFCDtsPath), { recursive: true });
                }
                await fs__namespace.promises.writeFile(vueSFCDtsPath, vueSFCDeclaration);
            }
            else if (options.write) {
                const vueSFCDtsPath = path__namespace.join(projectPath, relativePathDir, vueSFCDtsFileName);
                if (!fs__namespace.existsSync(path__namespace.dirname(vueSFCDtsPath))) {
                    fs__namespace.mkdirSync(path__namespace.dirname(vueSFCDtsPath), { recursive: true });
                }
                await fs__namespace.promises.writeFile(vueSFCDtsPath, vueSFCDeclaration);
    ...
    const outputChunks = await Promise.all(vueSFCFiles.map(async (vueSFCFile) => {
        const bundle = await rollup.rollup({
            plugins: [vue__default["default"]({ reactivityTransform: true }), styles__default["default"](), jsImports__default["default"]()],
            external(id) {
                // TODO: find a better way to distinguish between a project's import and a rollup plugin's import
                if (id.startsWith('plugin-vue') ||
        ...
        const relativePathDir = path__namespace.dirname(path__namespace.relative(projectPath, vueSFCFile));
        const vueSFCName = path__namespace.parse(vueSFCFile).name;
        const compiledVueSFCFileName = options.vueExtension
            ? `${vueSFCName}.vue.js`
            : `${vueSFCName}.js`;
        if (options.outDir !== undefined) {
            const compiledVueSFCPath = path__namespace.join(options.outDir, relativePathDir, compiledVueSFCFileName);
            if (!fs__namespace.existsSync(path__namespace.dirname(compiledVueSFCPath))) {
                fs__namespace.mkdirSync(path__namespace.dirname(compiledVueSFCPath), { recursive: true });
            }
            await fs__namespace.promises.writeFile(compiledVueSFCPath, code);
        }
        else if (options.write) {
            const compiledVueSFCPath = path__namespace.join(projectPath, relativePathDir, compiledVueSFCFileName);
            await fs__namespace.promises.writeFile(compiledVueSFCPath, code);
        }
